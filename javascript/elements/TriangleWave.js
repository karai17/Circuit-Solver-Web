        ...
        if (this.elm.consistent()) {
            engine_functions.stamp_voltage(this.elm.n1, this.elm.n2, this.elm.properties['Offset'] +
                ((2 * this.elm.properties['Voltage']) / Math.PI) *
                    Math.asin(Math.sin(2 * simulation_manager.simulation_time * Math.PI * this.elm.properties['Frequency'] + global.utils.to_radians(this.elm.properties['Phase']))), simulation_manager.ELEMENT_TRI_OFFSET + this.simulation_id);
        }
    }
    get_vertices() {
        let vertices = [];
