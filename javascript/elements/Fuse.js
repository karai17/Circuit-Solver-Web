...
class Fuse {
    constructor(type, id, n1, n2) {
        ...
        this.elm = new Element2(id, type, global.utils.copy(global.PROPERTY.PROPERTY_FUSE));
        ...
        if (global.flags.flag_simulating && simulation_manager.solutions_ready && simulation_manager.simulation_step !== 0) {
            if (this.elm.consistent()) {
                ...
                if (this.elm.properties['Voltage'] / this.elm.properties['Resistance'] >= this.elm.properties['Current Rating']) {
                    this.elm.properties['Broken'] = true;
                }
            }
        }
        else {
    ...
    draw_component(canvas) {
        ...
        if (global.flags.flag_picture_request ||
            ...
            this.circle_buffer = [];
            this.line_buffer = [];
            if (this.elm.properties['Broken'] === false) {
                canvas.draw_arc2(this.connect1_x, this.connect1_y, this.c_x, this.c_y, this.bounds.get_height() >> 2, this.line_paint);
                canvas.draw_arc2(this.c_x, this.c_y, this.connect2_x, this.connect2_y, -(this.bounds.get_height() >> 2), this.line_paint);
            }
            else {
                this.circle_buffer[this.indexer++] = Array(this.connect1_x, this.connect1_y, global.variables.canvas_stroke_width_2_zoom);
                this.circle_buffer[this.indexer++] = Array(this.connect2_x, this.connect2_y, global.variables.canvas_stroke_width_2_zoom);
            }
            ...
            if (global.variables.workspace_zoom_scale > 1.085 || (!MOBILE_MODE && global.variables.workspace_zoom_scale >= 0.99)) {
                ...
                if ((this.angle > 170 && this.angle < 190) || (this.angle > -10 && this.angle < 10)) {
                    canvas.draw_text(global.TEMPLATES.ELEMENT_VAL_TEMPLATE.replace('{VAL}', global.utils.exponentiate_quickly(this.elm.properties['Current Rating'])).replace('{UNIT}', this.elm.properties['units']), this.c_x, this.bounds.top + this.bounds.get_height() * 0.1, this.text_paint);
