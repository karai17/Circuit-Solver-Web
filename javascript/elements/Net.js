...
class Net {
    constructor(type, id, n1) {
        ...
        this.elm = new Element1(id, type, global.utils.copy(global.PROPERTY.PROPERTY_NET));
        ...
        if (global.flags.flag_picture_request ||
            ...
            if (this.elm.rotation === global.CONSTANTS.ROTATION_0) {
                ...
                if (!this.is_selected_element() && !this.multi_selected) {
                    this.point_paint.set_color(global.utils.unique_color(this.elm.properties['Name']));
                }
            ...
            if (global.variables.workspace_zoom_scale > 1.085 || (!MOBILE_MODE && global.variables.workspace_zoom_scale >= 0.99)) {
                if (this.elm.properties['Show Name'] === global.CONSTANTS.ON) {
                    if (this.elm.rotation === global.CONSTANTS.ROTATION_90) {
                        ...
                        canvas.draw_text(global.TEMPLATES.ELEMENT_VAL_TEMPLATE.replace('{VAL}', this.elm.properties['Name']).replace('{UNIT}', this.elm.properties['units']), this.c_x, this.bounds.bottom - this.bounds.get_height() * 0.2, this.text_paint);
                    }
                    else if (this.elm.rotation === global.CONSTANTS.ROTATION_0) {
                        canvas.rotate(this.bounds.left, this.c_y, -90);
                        canvas.draw_text(global.TEMPLATES.ELEMENT_VAL_TEMPLATE.replace('{VAL}', this.elm.properties['Name']).replace('{UNIT}', this.elm.properties['units']), this.bounds.left, this.c_y + this.bounds.get_height() * 0.2, this.text_paint);
                        canvas.restore();
                    }
                    else if (this.elm.rotation === global.CONSTANTS.ROTATION_180) {
                        canvas.rotate(this.bounds.right, this.c_y, -90);
                        canvas.draw_text(global.TEMPLATES.ELEMENT_VAL_TEMPLATE.replace('{VAL}', this.elm.properties['Name']).replace('{UNIT}', this.elm.properties['units']), this.bounds.right, this.c_y - this.bounds.get_height() * 0.2, this.text_paint);
                        canvas.restore();
                    }
                }
            }
    ...
    patch() {
        if (!global.utils.not_null(this.elm.properties['Show Name'])) {
            this.elm.properties['Show Name'] = global.CONSTANTS.ON;
        }
