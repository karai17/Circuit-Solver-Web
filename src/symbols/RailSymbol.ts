...
class RailSymbol {
	...
	draw_symbol(canvas: GraphicsEngine, page: number) {
		...
		if (this.page === page) {
			...
			this.circle_buffer = [];
			this.line_buffer = [];
			canvas.draw_circle(this.c_x, this.c_y + this.bounds.get_height() * 0.5, 1.5 * global.variables.canvas_stroke_width_2, this.point_paint);
			...
			this.line_buffer[indexer++] = Array(
				...
				this.c_y + this.bounds.get_height() * 0.5 - 2 * this.y_space,
				this.c_x + this.x_space,
				this.c_y + this.bounds.get_height() * 0.5 - 2 * this.y_space
			);
