...
class ANDGateSymbol {
	...
	constructor(rect: RectF, index: number, page: number) {
		...
		this.TAG = language_manager.TAG_AND;
		this.draw_tag = false;
	...
	build_element() {
		this.and_0.x = this.p1.x + 1.5 * this.x_space * global.utils.cosine(this.theta);
		this.and_0.y = this.p1.y + 1.5 * this.y_space * global.utils.sine(this.theta);
		this.and_1.x = this.and_0.x + 0.75 * this.x_space * global.utils.cosine(this.theta_m90);
		this.and_1.y = this.and_0.y + 0.75 * this.y_space * global.utils.sine(this.theta_m90);
		this.and_2.x = this.and_1.x + 0.75 * this.x_space * global.utils.cosine(this.theta - Math.PI);
		this.and_2.y = this.and_1.y + 0.75 * this.y_space * global.utils.sine(this.theta - Math.PI);
		this.and_3.x = this.p1.x + 2.5 * this.x_space * global.utils.cosine(this.theta);
		this.and_3.y = this.p1.y + 2.5 * this.y_space * global.utils.sine(this.theta);
		this.and_4.x = this.and_3.x + 0.75 * this.x_space * global.utils.cosine(this.theta_m90);
		this.and_4.y = this.and_3.y + 0.75 * this.y_space * global.utils.sine(this.theta_m90);
		this.and_5.x = this.and_4.x + 0.75 * this.x_space * global.utils.cosine(this.theta);
		this.and_5.y = this.and_4.y + 0.75 * this.y_space * global.utils.sine(this.theta);
		this.and_6.x = this.p3.x - 0.75 * this.x_space * global.utils.cosine(this.theta_m90);
		this.and_6.y = this.p3.y - 0.75 * this.y_space * global.utils.sine(this.theta_m90);
	}
	resize(rect: RectF) {
		this.bounds.set_bounds(rect.left, rect.top, rect.right, rect.bottom);
		this.c_x = this.bounds.get_center_x();
	...
	draw_symbol(canvas: GraphicsEngine, page: number) {
		...
		if (this.page === page) {
			...
			this.circle_buffer = [];
			this.line_buffer = [];
			this.line_buffer[indexer++] = Array(this.p1.x, this.p1.y, this.and_0.x, this.and_0.y);
			this.line_buffer[indexer++] = Array(this.and_0.x, this.and_0.y, this.and_1.x, this.and_1.y);
			this.line_buffer[indexer++] = Array(this.p2.x, this.p2.y, this.and_3.x, this.and_3.y);
			this.line_buffer[indexer++] = Array(this.and_3.x, this.and_3.y, this.and_4.x, this.and_4.y);
			this.line_buffer[indexer++] = Array(this.and_2.x, this.and_2.y, this.and_5.x, this.and_5.y);
			this.line_buffer[indexer++] = Array(this.and_6.x, this.and_6.y, this.p3.x, this.p3.y);
