'use strict';
class PulseWidthModulatorSymbol {
	...
	private p2: PointF;
	private p3: PointF;
	private pwm_0: PointF;
	private pwm_1: PointF;
	private pwm_2: PointF;
	private pwm_3: PointF;
	private pwm_4: PointF;
	private pwm_5: PointF;
	private pwm_6: PointF;
	private c_x: number;
	private c_y: number;
	...
	constructor(rect: RectF, index: number, page: number) {
		...
		this.TAG = language_manager.TAG_PWM;
		this.draw_tag = false;
	...
	build_element() {
		this.pwm_0.x = this.p1.x + 2.0 * this.x_space * global.utils.cosine(this.theta_m90);
		this.pwm_0.y = this.p1.y + 2.0 * this.y_space * global.utils.sine(this.theta_m90);
		this.pwm_1.x = this.pwm_0.x + this.x_space * global.utils.cosine(this.theta);
		this.pwm_1.y = this.pwm_0.y + this.y_space * global.utils.sine(this.theta);
		this.pwm_2.x = this.pwm_1.x + 0.75 * this.x_space * global.utils.cosine(this.theta - Math.PI);
		this.pwm_2.y = this.pwm_1.y + 0.75 * this.y_space * global.utils.sine(this.theta - Math.PI);
		this.pwm_3.x = this.p2.x + 2.0 * this.x_space * global.utils.cosine(this.theta_m90);
		this.pwm_3.y = this.p2.y + 2.0 * this.y_space * global.utils.sine(this.theta_m90);
		this.pwm_4.x = this.pwm_3.x - this.x_space * global.utils.cosine(this.theta);
		this.pwm_4.y = this.pwm_3.y - this.y_space * global.utils.sine(this.theta);
		this.pwm_5.x = this.pwm_4.x + 0.75 * this.x_space * global.utils.cosine(this.theta);
		this.pwm_5.y = this.pwm_4.y + 0.75 * this.y_space * global.utils.sine(this.theta);
		this.pwm_6.x = this.p3.x - this.x_space * global.utils.cosine(this.theta_m90);
		this.pwm_6.y = this.p3.y - this.y_space * global.utils.sine(this.theta_m90);
	}
	resize(rect: RectF) {
		this.bounds.set_bounds(rect.left, rect.top, rect.right, rect.bottom);
		this.c_x = this.bounds.get_center_x();
	...
	draw_symbol(canvas: GraphicsEngine, page: number) {
		...
		if (this.page === page) {
			...
			this.circle_buffer = [];
			this.line_buffer = [];
			this.line_buffer[indexer++] = Array(this.p1.x, this.p1.y, this.pwm_0.x, this.pwm_0.y);
			this.line_buffer[indexer++] = Array(this.pwm_0.x, this.pwm_0.y, this.pwm_1.x, this.pwm_1.y);
			this.line_buffer[indexer++] = Array(this.p2.x, this.p2.y, this.pwm_3.x, this.pwm_3.y);
			this.line_buffer[indexer++] = Array(this.pwm_3.x, this.pwm_3.y, this.pwm_4.x, this.pwm_4.y);
			this.line_buffer[indexer++] = Array(this.c_x - (this.bounds.get_width() >> 3), this.c_y + (this.bounds.get_height() >> 3), this.c_x, this.c_y + (this.bounds.get_height() >> 3));
			this.line_buffer[indexer++] = Array(this.c_x, this.c_y + (this.bounds.get_height() >> 3), this.c_x, this.c_y - (this.bounds.get_height() >> 3));
			this.line_buffer[indexer++] = Array(this.c_x, this.c_y - (this.bounds.get_height() >> 3), this.c_x + (this.bounds.get_width() >> 3), this.c_y - (this.bounds.get_height() >> 3));
			this.line_buffer[indexer++] = Array(this.pwm_6.x, this.pwm_6.y, this.p3.x, this.p3.y);
