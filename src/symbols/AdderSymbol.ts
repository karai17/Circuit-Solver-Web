...
class AdderSymbol {
	...
	constructor(rect: RectF, index: number, page: number) {
		...
		this.TAG = language_manager.TAG_ADD;
		this.draw_tag = false;
	...
	build_element() {
		this.adder_0.x = this.p1.x + 2.0 * this.x_space * global.utils.cosine(this.theta_m90);
		this.adder_0.y = this.p1.y + 2.0 * this.y_space * global.utils.sine(this.theta_m90);
		this.adder_1.x = this.adder_0.x + this.x_space * global.utils.cosine(this.theta);
		this.adder_1.y = this.adder_0.y + this.y_space * global.utils.sine(this.theta);
		this.adder_2.x = this.adder_1.x + 0.75 * this.x_space * global.utils.cosine(this.theta - Math.PI);
		this.adder_2.y = this.adder_1.y + 0.75 * this.y_space * global.utils.sine(this.theta - Math.PI);
		this.adder_3.x = this.p2.x + 2.0 * this.x_space * global.utils.cosine(this.theta_m90);
		this.adder_3.y = this.p2.y + 2.0 * this.y_space * global.utils.sine(this.theta_m90);
		this.adder_4.x = this.adder_3.x - this.x_space * global.utils.cosine(this.theta);
		this.adder_4.y = this.adder_3.y - this.y_space * global.utils.sine(this.theta);
		this.adder_5.x = this.adder_4.x + 0.75 * this.x_space * global.utils.cosine(this.theta);
		this.adder_5.y = this.adder_4.y + 0.75 * this.y_space * global.utils.sine(this.theta);
		this.adder_6.x = this.p3.x - 0.75 * this.x_space * global.utils.cosine(this.theta_m90);
		this.adder_6.y = this.p3.y - 0.75 * this.y_space * global.utils.sine(this.theta_m90);
	}
	resize(rect: RectF) {
		this.bounds.set_bounds(rect.left, rect.top, rect.right, rect.bottom);
		this.c_x = this.bounds.get_center_x();
	...
	draw_symbol(canvas: GraphicsEngine, page: number) {
		...
		if (this.page === page) {
			...
			this.circle_buffer = [];
			this.line_buffer = [];
			this.line_buffer[indexer++] = Array(this.p1.x, this.p1.y, this.adder_0.x, this.adder_0.y);
			this.line_buffer[indexer++] = Array(this.adder_0.x, this.adder_0.y, this.adder_1.x, this.adder_1.y);
			this.line_buffer[indexer++] = Array(this.p2.x, this.p2.y, this.adder_3.x, this.adder_3.y);
			this.line_buffer[indexer++] = Array(this.adder_3.x, this.adder_3.y, this.adder_4.x, this.adder_4.y);
			this.line_buffer[indexer++] = Array(this.c_x - this.bounds.get_width() * 0.1, this.c_y, this.c_x + this.bounds.get_width() * 0.1, this.c_y);
			this.line_buffer[indexer++] = Array(this.c_x, this.c_y + this.bounds.get_width() * 0.1, this.c_x, this.c_y - this.bounds.get_width() * 0.1);
			this.line_buffer[indexer++] = Array(this.adder_6.x, this.adder_6.y, this.p3.x, this.p3.y);
