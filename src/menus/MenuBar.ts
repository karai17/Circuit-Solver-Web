...
class MenuBar {
	...
	private line_paint_alt: Paint;
	private fill_paint_alt: Paint;
	private up_down_paint: Paint;
	private add_paint: Paint;
	private add_paint_alt: Paint;
	private go_paint: Paint;
	private hover_paint: Paint;
	private undo_paint: Paint;
	private redo_paint: Paint;
	private remove_all_paint: Paint;
	private settings_paint: Paint;
	private zoom_paint: Paint;
	private save_circuit_paint: Paint;
	private save_image_fill_paint: Paint;
	private text_paint: Paint;
	private save_ckt_path1: Path;
	private save_ckt_path2: Path;
	private go_path: Path;
	private undo_path: Path;
	private redo_path: Path;
	private save_img_path: Path;
	public graph_button: RectF;
	public settings_button: RectF;
	private sine_wave: SineWave;
	...
	private save_x1: string;
	private save_y1: string;
	private save_x2: string;
	private save_y2: string;
	private go_x: string;
	private go_y: string;
	private undo_x: string;
	private undo_y: string;
	private redo_x: string;
	private redo_y: string;
	private save_image_x0: string;
	private save_image_y0: string;

	constructor() {
		let temp_stroke_width: number = 0.70 * global.variables.canvas_stroke_width_3;
		this.MAX_ICONS = 8;
		if (MOBILE_MODE) {
		...
		this.element_window = new ElementWindow(
			this.bounds.left,
			this.bounds.bottom + (global.variables.canvas_stroke_width_4 >> 1),
			this.bounds.right,
			this.bounds.bottom + this.bounds.get_height() - (global.variables.canvas_stroke_width_4 >> 1)
		);
	...
	load_icons(): void {
		...
		this.graph_button.set_bounds(
			...
			this.menu_icons[this.REMOVE_ALL_INDEX].bottom + 2 * global.variables.canvas_stroke_width_4,
			this.menu_icons[this.REMOVE_ALL_INDEX].right,
			this.menu_icons[this.REMOVE_ALL_INDEX].bottom + 3 * global.variables.canvas_stroke_width_4 + this.menu_icons[this.REMOVE_ALL_INDEX].get_height() - global.variables.canvas_stroke_width_4
		);
		...
		this.sine_wave.set_points(
			this.graph_button.left + this.graph_button.get_width() * 0.275,
			this.graph_button.bottom - this.graph_button.get_height() * 0.275,
			this.graph_button.right - this.graph_button.get_width() * 0.275,
			this.graph_button.get_center_y() - this.graph_button.get_height() * 0.2
		);
	...
	draw_menu_bar(canvas: GraphicsEngine): void {
		...
		if (global.flags.flag_menu_open) {
			...
			canvas.draw_arc3(
				...
				0.45 * this.menu_icons[this.REMOVE_ALL_INDEX].get_width(),
				-25,
				290,
			...
			canvas.draw_rect3(this.menu_icons[this.ADD_INDEX].get_center_x(), this.menu_icons[this.ADD_INDEX].get_center_y(),
				this.menu_icons[this.ADD_INDEX].get_width() * 0.85, this.menu_icons[this.ADD_INDEX].get_height() * 0.85, this.add_paint);
			this.line_buffer[this.indexer++] = Array(
		...
		this.line_buffer[this.indexer++] = Array(
			this.settings_button.left + padding_cache_w,
			this.settings_button.top + padding_cache_h,
			this.settings_button.right - padding_cache_w,
			this.settings_button.top + padding_cache_h
		);
		...
		this.line_buffer[this.indexer++] = Array(
			this.settings_button.left + padding_cache_w,
			this.settings_button.bottom - padding_cache_h,
			this.settings_button.right - padding_cache_w,
			this.settings_button.bottom - padding_cache_h
		);
		...
		canvas.draw_circle(
			this.settings_button.left + padding_cache_w + (width_cache - (padding_cache_w << 1)) * 0.20,
			this.settings_button.top + padding_cache_h,
			padding_cache_w * 0.2,
			this.settings_paint
		);
		...
		canvas.draw_circle(
			this.settings_button.left + padding_cache_w + (width_cache - (padding_cache_w << 1)) * 0.80,
			this.settings_button.get_center_y(),
			padding_cache_w * 0.2,
			this.settings_paint
		);
