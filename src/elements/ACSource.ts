...
class ACSource {
	...
	constructor(type: number, id: number, n1: number, n2: number) {
		...
		this.elm = new Element2(id, type, global.utils.copy(global.PROPERTY.PROPERTY_ACSOURCE));
	...
	build_element(): void {
		if (
			...
			this.sine_wave_p1.x = this.c_x - (cache_2 >> 1) * global.utils.cosine(Math.PI + this.theta_m90);
			this.sine_wave_p1.y = this.c_y - (cache_3 >> 1) * global.utils.sine(Math.PI + this.theta_m90);
			this.sine_wave_p2.x = this.c_x + (cache_2 >> 1) * global.utils.cosine(Math.PI + this.theta_m90);
			this.sine_wave_p2.y = this.c_y + (cache_3 >> 1) * global.utils.sine(Math.PI + this.theta_m90);
			this.sine_wave.set_points(this.sine_wave_p1.x, this.sine_wave_p1.y, this.sine_wave_p2.x, this.sine_wave_p2.y);
			this.sine_wave.set_amplitude(cache_2 >> 1);
