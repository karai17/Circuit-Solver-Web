...
class XNORGate {
	...
	private p2: PointF;
	private p3: PointF;
	private xnor_0: PointF;
	private xnor_1: PointF;
	private xnor_2: PointF;
	private xnor_3: PointF;
	private xnor_4: PointF;
	private xnor_5: PointF;
	private xnor_6: PointF;
	private xnor_7: PointF;
	private xnor_8: PointF;
	private xnor_9: PointF;
	private xnor_10: PointF;
	private xnor_11: PointF;
	private xnor_12: PointF;
	...
	vout_xnor(ui: Array<number>): number {
		let product = 1;
	...
	partial_xnor(terminal: number, ui: Array<number>, ui_prime: Array<number>): number {
		let product = 1;
		...
		if (global.flags.flag_simulating && simulation_manager.solutions_ready) {
			if (this.elm.consistent()) {
				...
				this.elm.properties['V_partial1'] = global.utils.limit(
					this.partial_xnor(0, [this.elm.properties['V_1'], this.elm.properties['V_2']], [this.elm.properties['V_1_prime'], this.elm.properties['V_2_prime']]),
					0.0,
					1.0
				);
				this.elm.properties['V_partial2'] = global.utils.limit(
					this.partial_xnor(1, [this.elm.properties['V_1'], this.elm.properties['V_2']], [this.elm.properties['V_1_prime'], this.elm.properties['V_2_prime']]),
					0.0,
					1.0
				);
	...
	stamp(): void {
		if (this.elm.consistent()) {
			engine_functions.stamp_gate2(
				...
				this.elm.properties['V_eq'],
				simulation_manager.ELEMENT_XNOR_OFFSET + this.simulation_id
			);
		}
	}
	...
	build_element(): void {
		if (
			...
			let cache_10: number = 0.9 * this.x_space;
			let cache_11: number = 0.9 * this.y_space;
			let cache_12: number = 0.6 * this.x_space;
			let cache_13: number = 0.6 * this.y_space;
			let cache_14: number = 0.3 * this.x_space;
			let cache_15: number = 0.3 * this.y_space;
			let cache_16: number = this.x_space;
			let cache_17: number = this.y_space;
