...
class NANDGate {
	...
	private p2: PointF;
	private p3: PointF;
	private nand_0: PointF;
	private nand_1: PointF;
	private nand_2: PointF;
	private nand_3: PointF;
	private nand_4: PointF;
	private nand_5: PointF;
	private nand_6: PointF;
	private nand_7: PointF;
	private nand_8: PointF;
	...
	constructor(type: number, id: number, n1: number, n2: number, n3: number) {
		...
		this.elm = new Element3(id, type, global.utils.copy(global.PROPERTY.PROPERTY_NAND));
	...
	stamp(): void {
		if (this.elm.consistent()) {
			engine_functions.stamp_voltage(this.elm.n3, -1, this.elm.properties['Output Voltage'], simulation_manager.ELEMENT_NAND_OFFSET + this.simulation_id);
		}
	}
	...
	build_element(): void {
		if (
			...
			let cache_6: number = 0.9 * this.x_space;
			let cache_7: number = 0.9 * this.y_space;
			let cache_8: number = 0.6 * this.x_space;
			let cache_9: number = 0.6 * this.y_space;
			let cache_10: number = 0.3 * this.x_space;
			let cache_11: number = 0.3 * this.y_space;
			let cache_12: number = this.x_space;
			let cache_13: number = this.y_space;
			this.nand_0.x = this.p1.x + cache_0 * global.utils.cosine(this.theta);
			this.nand_0.y = this.p1.y + cache_1 * global.utils.sine(this.theta);
			this.nand_1.x = this.nand_0.x + cache_2 * global.utils.cosine(this.theta_m90);
			this.nand_1.y = this.nand_0.y + cache_3 * global.utils.sine(this.theta_m90);
			this.nand_2.x = this.nand_1.x + cache_2 * global.utils.cosine(this.theta - Math.PI);
			this.nand_2.y = this.nand_1.y + cache_3 * global.utils.sine(this.theta - Math.PI);
			this.nand_3.x = this.p1.x + cache_4 * global.utils.cosine(this.theta);
			this.nand_3.y = this.p1.y + cache_5 * global.utils.sine(this.theta);
			this.nand_4.x = this.nand_3.x + cache_2 * global.utils.cosine(this.theta_m90);
			this.nand_4.y = this.nand_3.y + cache_3 * global.utils.sine(this.theta_m90);
			this.nand_5.x = this.nand_4.x + cache_2 * global.utils.cosine(this.theta);
			this.nand_5.y = this.nand_4.y + cache_3 * global.utils.sine(this.theta);
			this.nand_6.x = this.p3.x - cache_6 * global.utils.cosine(this.theta_m90);
			this.nand_6.y = this.p3.y - cache_7 * global.utils.sine(this.theta_m90);
			this.nand_7.x = this.p3.x - cache_8 * global.utils.cosine(this.theta_m90);
			this.nand_7.y = this.p3.y - cache_9 * global.utils.sine(this.theta_m90);
			this.nand_8.x = this.p3.x - cache_10 * global.utils.cosine(this.theta_m90);
			this.nand_8.y = this.p3.y - cache_11 * global.utils.sine(this.theta_m90);
	...
	draw_component(canvas: GraphicsEngine): void {
		...
		if (
			...
			this.circle_buffer = [];
			this.line_buffer = [];
			this.line_buffer[this.indexer++] = Array(this.p1.x, this.p1.y, this.nand_0.x, this.nand_0.y);
			this.line_buffer[this.indexer++] = Array(this.nand_0.x, this.nand_0.y, this.nand_1.x, this.nand_1.y);
			this.line_buffer[this.indexer++] = Array(this.p2.x, this.p2.y, this.nand_3.x, this.nand_3.y);
			this.line_buffer[this.indexer++] = Array(this.nand_3.x, this.nand_3.y, this.nand_4.x, this.nand_4.y);
			this.line_buffer[this.indexer++] = Array(this.nand_2.x, this.nand_2.y, this.nand_5.x, this.nand_5.y);
			this.line_buffer[this.indexer++] = Array(this.nand_8.x, this.nand_8.y, this.p3.x, this.p3.y);
