...
class ORGate {
	...
	constructor(type: number, id: number, n1: number, n2: number, n3: number) {
		...
		this.elm = new Element3(id, type, global.utils.copy(global.PROPERTY.PROPERTY_OR));
	...
	vout_or(ui: Array<number>): number {
		let sum = 0;
		let N = 2;
		let size = ui.length;
	...
	partial_or(terminal: number, ui: Array<number>, ui_prime: Array<number>): number {
		let sum = 0;
		let N = 2;
		let size = ui.length;
	...
	update(): void {
		if (global.flags.flag_simulating && simulation_manager.solutions_ready) {
			if (this.elm.consistent()) {
				...
				this.elm.properties['V_partial1'] = global.utils.limit(this.partial_or(0, [this.elm.properties['V_1'], this.elm.properties['V_2']],
					[this.elm.properties['V_1_prime'], this.elm.properties['V_2_prime']]), 0.0, 1.0);
				this.elm.properties['V_partial2'] = global.utils.limit(this.partial_or(1, [this.elm.properties['V_1'], this.elm.properties['V_2']],
					[this.elm.properties['V_1_prime'], this.elm.properties['V_2_prime']]), 0.0, 1.0);
	...
	stamp(): void {
		if (this.elm.consistent()) {
			engine_functions.stamp_gate2(this.elm.n3, this.elm.properties['V_partial1'], this.elm.properties['V_partial2'], this.elm.properties['V_eq'], simulation_manager.ELEMENT_OR_OFFSET + this.simulation_id);
		}
	}
	...
	build_element(): void {
		if (
			...
			this.or_0.x = this.p1.x + cache_0 * global.utils.cosine(this.theta);
			this.or_0.y = this.p1.y + cache_1 * global.utils.sine(this.theta);
			this.or_1.x = this.or_0.x + cache_2 * global.utils.cosine(this.theta_m90);
			this.or_1.y = this.or_0.y + cache_3 * global.utils.sine(this.theta_m90);
			this.or_2.x = this.or_1.x + cache_2 * global.utils.cosine(this.theta - Math.PI);
			this.or_2.y = this.or_1.y + cache_3 * global.utils.sine(this.theta - Math.PI);
			this.or_7.x = this.or_0.x + cache_4 * global.utils.cosine(this.theta_m90);
			this.or_7.y = this.or_0.y + cache_5 * global.utils.sine(this.theta_m90);
			this.or_3.x = this.p1.x + cache_6 * global.utils.cosine(this.theta);
			this.or_3.y = this.p1.y + cache_7 * global.utils.sine(this.theta);
			this.or_4.x = this.or_3.x + cache_2 * global.utils.cosine(this.theta_m90);
			this.or_4.y = this.or_3.y + cache_3 * global.utils.sine(this.theta_m90);
			this.or_5.x = this.or_4.x + cache_2 * global.utils.cosine(this.theta);
			this.or_5.y = this.or_4.y + cache_3 * global.utils.sine(this.theta);
			this.or_8.x = this.or_3.x + cache_4 * global.utils.cosine(this.theta_m90);
			this.or_8.y = this.or_3.y + cache_5 * global.utils.sine(this.theta_m90);
			this.or_6.x = this.p3.x - cache_2 * global.utils.cosine(this.theta_m90);
			this.or_6.y = this.p3.y - cache_3 * global.utils.sine(this.theta_m90);
	...
	draw_component(canvas: GraphicsEngine): void {
		...
		if (
			...
			this.circle_buffer = [];
			this.line_buffer = [];
			this.line_buffer[this.indexer++] = Array(this.p1.x, this.p1.y, this.or_0.x, this.or_0.y);
			this.line_buffer[this.indexer++] = Array(this.or_0.x, this.or_0.y, this.or_7.x, this.or_7.y);
			this.line_buffer[this.indexer++] = Array(this.p2.x, this.p2.y, this.or_3.x, this.or_3.y);
			this.line_buffer[this.indexer++] = Array(this.or_3.x, this.or_3.y, this.or_8.x, this.or_8.y);
			this.line_buffer[this.indexer++] = Array(this.or_6.x, this.or_6.y, this.p3.x, this.p3.y);
