...
class ANDGate {
	...
	private p2: PointF;
	private p3: PointF;
	private and_0: PointF;
	private and_1: PointF;
	private and_2: PointF;
	private and_3: PointF;
	private and_4: PointF;
	private and_5: PointF;
	private and_6: PointF;
	...
	constructor(type: number, id: number, n1: number, n2: number, n3: number) {
		...
		this.elm = new Element3(id, type, global.utils.copy(global.PROPERTY.PROPERTY_AND));
		...
		this.and_0 = new PointF(0, 0);
		this.and_1 = new PointF(0, 0);
		this.and_2 = new PointF(0, 0);
		this.and_3 = new PointF(0, 0);
	...
	vout_and(ui: Array<number>): number {
		let sum = 0;
		let N = 2;
		let size = ui.length;
		...
		return (N / sum) + global.CONSTANTS.ZERO_BIAS;
	}
	partial_and(terminal: number, ui: Array<number>, ui_prime: Array<number>): number {
		let sum = 0;
		let N = 2;
		let size = ui.length;
	...
	update(): void {
		if (global.flags.flag_simulating && simulation_manager.solutions_ready) {
			if (this.elm.consistent()) {
				...
				this.elm.properties['V_out'] = this.vout_and([this.elm.properties['V_1'], this.elm.properties['V_2']]);
				this.elm.properties['V_partial1'] = global.utils.limit(this.partial_and(0, [this.elm.properties['V_1'], this.elm.properties['V_2']],
					[this.elm.properties['V_1_prime'], this.elm.properties['V_2_prime']]), 0.0, 1.0);
				this.elm.properties['V_partial2'] = global.utils.limit(this.partial_and(1, [this.elm.properties['V_1'], this.elm.properties['V_2']],
					[this.elm.properties['V_1_prime'], this.elm.properties['V_2_prime']]), 0.0, 1.0);
	...
	stamp(): void {
		if (this.elm.consistent()) {
			engine_functions.stamp_gate2(this.elm.n3, this.elm.properties['V_partial1'], this.elm.properties['V_partial2'], this.elm.properties['V_eq'], simulation_manager.ELEMENT_AND_OFFSET + this.simulation_id);
		}
	}
	...
	build_element(): void {
		if (
			...
			this.and_0.x = this.p1.x + cache_0 * global.utils.cosine(this.theta);
			this.and_0.y = this.p1.y + cache_1 * global.utils.sine(this.theta);
			this.and_1.x = this.and_0.x + cache_2 * global.utils.cosine(this.theta_m90);
			this.and_1.y = this.and_0.y + cache_3 * global.utils.sine(this.theta_m90);
			this.and_2.x = this.and_1.x + cache_2 * global.utils.cosine(this.theta - Math.PI);
			this.and_2.y = this.and_1.y + cache_3 * global.utils.sine(this.theta - Math.PI);
			this.and_3.x = this.p1.x + cache_4 * global.utils.cosine(this.theta);
			this.and_3.y = this.p1.y + cache_5 * global.utils.sine(this.theta);
			this.and_4.x = this.and_3.x + cache_2 * global.utils.cosine(this.theta_m90);
			this.and_4.y = this.and_3.y + cache_3 * global.utils.sine(this.theta_m90);
			this.and_5.x = this.and_4.x + cache_2 * global.utils.cosine(this.theta);
			this.and_5.y = this.and_4.y + cache_3 * global.utils.sine(this.theta);
			this.and_6.x = this.p3.x - cache_2 * global.utils.cosine(this.theta_m90);
			this.and_6.y = this.p3.y - cache_3 * global.utils.sine(this.theta_m90);
	...
	draw_component(canvas: GraphicsEngine): void {
		...
		if (
			...
			this.circle_buffer = [];
			this.line_buffer = [];
			this.line_buffer[this.indexer++] = Array(this.p1.x, this.p1.y, this.and_0.x, this.and_0.y);
			this.line_buffer[this.indexer++] = Array(this.and_0.x, this.and_0.y, this.and_1.x, this.and_1.y);
			this.line_buffer[this.indexer++] = Array(this.p2.x, this.p2.y, this.and_3.x, this.and_3.y);
			this.line_buffer[this.indexer++] = Array(this.and_3.x, this.and_3.y, this.and_4.x, this.and_4.y);
			this.line_buffer[this.indexer++] = Array(this.and_2.x, this.and_2.y, this.and_5.x, this.and_5.y);
			this.line_buffer[this.indexer++] = Array(this.and_6.x, this.and_6.y, this.p3.x, this.p3.y);
			canvas.draw_line_buffer(this.line_buffer, this.line_paint);
			this.indexer = 0;
			canvas.draw_arc2(this.and_2.x, this.and_2.y, this.and_6.x, this.and_6.y, this.x_space, this.line_paint);
			canvas.draw_arc2(this.and_5.x, this.and_5.y, this.and_6.x, this.and_6.y, -this.x_space, this.line_paint);
