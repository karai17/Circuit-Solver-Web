...
class PNPBipolarJunctionTransistor {
	...
	private p2: PointF;
	private p3: PointF;
	private pnp_0: PointF;
	private pnp_1: PointF;
	private pnp_2: PointF;
	private pnp_3: PointF;
	private pnp_4: PointF;
	private pnp_5: PointF;
	private pnp_6: PointF;
	private pnp_7: PointF;
	private pnp_8: PointF;
	...
	private damping_safety_factor: number;
	...
	constructor(type: number, id: number, n1: number, n2: number, n3: number) {
		...
		this.elm = new Element3(id, type, global.utils.copy(global.PROPERTY.PROPERTY_PNP));
		...
		this.pnp_0 = new PointF(0, 0);
		this.pnp_1 = new PointF(0, 0);
		this.pnp_2 = new PointF(0, 0);
		this.pnp_3 = new PointF(0, 0);
		this.pnp_4 = new PointF(0, 0);
		this.pnp_5 = new PointF(0, 0);
		this.pnp_6 = new PointF(0, 0);
		this.pnp_7 = new PointF(0, 0);
		this.pnp_8 = new PointF(0, 0);
	...
	stamp(): void {
		if (this.elm.consistent()) {
			let b: number = global.utils.copy(this.elm.n3);
			let c: number = global.utils.copy(this.elm.n2);
			let e: number = global.utils.copy(this.elm.n1);
	...
	get_pnpbjt_error(): number {
		return Math.abs(this.elm.properties['Veb'] - this.elm.properties['Last Veb']);
	}
	update(): void {
		if (global.flags.flag_simulating && simulation_manager.solutions_ready) {
			if (this.elm.consistent()) {
				...
				if (next_veb > this.damping_safety_factor * vcrit) {
					veb = global.utils.log_damping(next_veb, this.elm.properties['Veb'], this.gamma, this.kappa);
				} else if (next_veb < -this.damping_safety_factor * vcrit) {
					veb = global.utils.log_damping(next_veb, this.elm.properties['Veb'], this.gamma, this.kappa);
				} else {
					veb = next_veb;
				}
				veb = global.utils.limit(veb, -vcrit, vcrit);
				...
				if (next_vcb > this.damping_safety_factor * vcrit) {
					vcb = global.utils.log_damping(next_vcb, this.elm.properties['Vcb'], this.gamma, this.kappa);
				} else if (next_vcb < -this.damping_safety_factor * vcrit) {
					vcb = global.utils.log_damping(next_vcb, this.elm.properties['Vcb'], this.gamma, this.kappa);
				} else {
					vcb = next_vcb;
				}
				vcb = global.utils.limit(vcb, -vcrit, vcrit);
				this.elm.properties['Vcb'] = vcb;
				...
				this.elm.properties['g_ee'] = (this.elm.properties['Saturation Current'] / global.settings.THERMAL_VOLTAGE) * Math.exp(this.elm.properties['Veb'] / global.settings.THERMAL_VOLTAGE);
				this.elm.properties['g_ec'] =
					-(reverse_alpha * (this.elm.properties['Saturation Current'] / global.settings.THERMAL_VOLTAGE)) * Math.exp(this.elm.properties['Vcb'] / global.settings.THERMAL_VOLTAGE);
				this.elm.properties['g_ce'] =
					-(forward_alpha * (this.elm.properties['Saturation Current'] / global.settings.THERMAL_VOLTAGE)) * Math.exp(this.elm.properties['Veb'] / global.settings.THERMAL_VOLTAGE);
				this.elm.properties['g_cc'] = (this.elm.properties['Saturation Current'] / global.settings.THERMAL_VOLTAGE) * Math.exp(this.elm.properties['Vcb'] / global.settings.THERMAL_VOLTAGE);
				this.elm.properties['i_e'] =
					this.elm.properties['Saturation Current'] * (Math.exp(this.elm.properties['Veb'] / global.settings.THERMAL_VOLTAGE) - 1) -
					reverse_alpha * this.elm.properties['Saturation Current'] * (Math.exp(this.elm.properties['Vcb'] / global.settings.THERMAL_VOLTAGE) - 1);
				this.elm.properties['i_c'] =
					this.elm.properties['Saturation Current'] * (Math.exp(this.elm.properties['Vcb'] / global.settings.THERMAL_VOLTAGE) - 1) -
					forward_alpha * this.elm.properties['Saturation Current'] * (Math.exp(this.elm.properties['Veb'] / global.settings.THERMAL_VOLTAGE) - 1);
	...
	build_element(): void {
		if (
			...
			let cache_0: number = 2.075 * this.x_space;
			let cache_1: number = 2.075 * this.y_space;
			...
			let cache_6: number = 2.1 * this.x_space;
			let cache_7: number = 2.1 * this.y_space;
			...
			this.pnp_0.x = this.p1.x + cache_10 * global.utils.cosine(this.theta);
			this.pnp_0.y = this.p1.y + cache_11 * global.utils.sine(this.theta);
			if (this.elm.flip === global.CONSTANTS.FLIP_180) {
				this.pnp_1.x = this.pnp_0.x + cache_0 * global.utils.cosine(this.theta_m90 + global.CONSTANTS.PI_DIV_12);
				this.pnp_1.y = this.pnp_0.y + cache_1 * global.utils.sine(this.theta_m90 + global.CONSTANTS.PI_DIV_12);
			} else if (this.elm.flip === global.CONSTANTS.FLIP_0) {
				this.pnp_1.x = this.pnp_0.x + cache_0 * global.utils.cosine(this.theta_m90 - global.CONSTANTS.PI_DIV_12);
				this.pnp_1.y = this.pnp_0.y + cache_1 * global.utils.sine(this.theta_m90 - global.CONSTANTS.PI_DIV_12);
			} else {
			...
			this.pnp_2.x = this.p1.x + cache_10 * global.utils.cosine(this.theta) + cache_2 * global.utils.cosine(this.theta_m90);
			this.pnp_2.y = this.p1.y + cache_11 * global.utils.sine(this.theta) + cache_3 * global.utils.sine(this.theta_m90);
			this.pnp_3.x = this.p1.x + cache_4 * global.utils.cosine(this.theta);
			this.pnp_3.y = this.p1.y + cache_5 * global.utils.sine(this.theta);
			if (this.elm.flip === global.CONSTANTS.FLIP_180) {
				this.pnp_4.x = this.pnp_3.x + cache_0 * global.utils.cosine(this.theta_m90 - global.CONSTANTS.PI_DIV_12);
				this.pnp_4.y = this.pnp_3.y + cache_1 * global.utils.sine(this.theta_m90 - global.CONSTANTS.PI_DIV_12);
			} else if (this.elm.flip === global.CONSTANTS.FLIP_0) {
				this.pnp_4.x = this.pnp_3.x + cache_0 * global.utils.cosine(this.theta_m90 + global.CONSTANTS.PI_DIV_12);
				this.pnp_4.y = this.pnp_3.y + cache_1 * global.utils.sine(this.theta_m90 + global.CONSTANTS.PI_DIV_12);
			} else {
			...
			this.pnp_5.x = this.p1.x + cache_4 * global.utils.cosine(this.theta) + cache_2 * global.utils.cosine(this.theta_m90);
			this.pnp_5.y = this.p1.y + cache_5 * global.utils.sine(this.theta) + cache_3 * global.utils.sine(this.theta_m90);
			this.pnp_6.x = this.p3.x - cache_6 * global.utils.cosine(this.theta_m90);
			this.pnp_6.y = this.p3.y - cache_7 * global.utils.sine(this.theta_m90);
			if (this.elm.flip === global.CONSTANTS.FLIP_180) {
				this.pnp_7.x = this.pnp_1.x - cache_8 * global.utils.cosine(this.theta_m90 + global.CONSTANTS.PI_DIV_4);
				this.pnp_7.y = this.pnp_1.y - cache_9 * global.utils.sine(this.theta_m90 + global.CONSTANTS.PI_DIV_4);
				this.pnp_8.x = this.pnp_1.x - cache_8 * global.utils.cosine(this.theta_m90 - global.CONSTANTS.PI_DIV_12);
				this.pnp_8.y = this.pnp_1.y - cache_9 * global.utils.sine(this.theta_m90 - global.CONSTANTS.PI_DIV_12);
			} else if (this.elm.flip === global.CONSTANTS.FLIP_0) {
				this.pnp_7.x = this.pnp_1.x - cache_8 * global.utils.cosine(this.theta_m90 - global.CONSTANTS.PI_DIV_4);
				this.pnp_7.y = this.pnp_1.y - cache_9 * global.utils.sine(this.theta_m90 - global.CONSTANTS.PI_DIV_4);
				this.pnp_8.x = this.pnp_1.x - cache_8 * global.utils.cosine(this.theta_m90 + global.CONSTANTS.PI_DIV_12);
		...
		if (
			...
			if (global.variables.workspace_zoom_scale > 1.085 || (!MOBILE_MODE && global.variables.workspace_zoom_scale >= 0.99)) {
				...
				if ((this.angle > 170 && this.angle < 190) || (this.angle > -10 && this.angle < 10)) {
					...
					canvas.draw_text(
						global.TEMPLATES.ELEMENT_VAL_TEMPLATE.replace('{VAL}', global.utils.exponentiate_quickly(this.elm.properties['Forward Beta'])).replace('{UNIT}', this.elm.properties['units']),
						this.c_x,
	...
	reset(): void {
		...
		this.elm.properties['Last Veb'] = this.calculate_vcrit();
