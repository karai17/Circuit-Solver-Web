...
class PulseWidthModulator {
	...
	private pwm_7: PointF;
	private pwm_8: PointF;
	...
	constructor(type: number, id: number, n1: number, n2: number, n3: number) {
		...
		this.elm = new Element3(id, type, global.utils.copy(global.PROPERTY.PROPERTY_PWM));
		...
		if (global.flags.flag_simulating && simulation_manager.solutions_ready && simulation_manager.simulation_step !== 0) {
			if (this.elm.consistent()) {
				...
				if (this.elm.properties['Saw Wave'] > 1.0 - this.elm.properties['Duty'] * 0.01) {
					...
					this.elm.properties['A'] = global.utils.copy(this.elm.properties['Low Voltage']);
				}
			}
		}
	}
	...
	build_element(): void {
		if (
			...
			this.pwm_0.x = this.p1.x + cache_0 * global.utils.cosine(this.theta_m90);
			this.pwm_0.y = this.p1.y + cache_1 * global.utils.sine(this.theta_m90);
			this.pwm_1.x = this.pwm_0.x + cache_6 * global.utils.cosine(this.theta);
			this.pwm_1.y = this.pwm_0.y + cache_7 * global.utils.sine(this.theta);
			this.pwm_2.x = this.pwm_1.x + cache_2 * global.utils.cosine(this.theta - Math.PI);
			this.pwm_2.y = this.pwm_1.y + cache_3 * global.utils.sine(this.theta - Math.PI);
			this.pwm_3.x = this.p2.x + cache_0 * global.utils.cosine(this.theta_m90);
			this.pwm_3.y = this.p2.y + cache_1 * global.utils.sine(this.theta_m90);
			this.pwm_4.x = this.pwm_3.x - cache_6 * global.utils.cosine(this.theta);
			this.pwm_4.y = this.pwm_3.y - cache_7 * global.utils.sine(this.theta);
			this.pwm_5.x = this.pwm_4.x + cache_2 * global.utils.cosine(this.theta);
			this.pwm_5.y = this.pwm_4.y + cache_3 * global.utils.sine(this.theta);
			this.pwm_6.x = this.p3.x - cache_6 * global.utils.cosine(this.theta_m90);
			this.pwm_6.y = this.p3.y - cache_7 * global.utils.sine(this.theta_m90);
			if (this.elm.flip === global.CONSTANTS.FLIP_0) {
				this.pwm_7.x = this.p1.x + cache_4 * global.utils.cosine(this.theta_m90 + global.CONSTANTS.PI_DIV_4);
				this.pwm_7.y = this.p1.y + cache_5 * global.utils.sine(this.theta_m90 + global.CONSTANTS.PI_DIV_4);
				this.pwm_8.x = this.p2.x + cache_4 * global.utils.cosine(this.theta_m90 - global.CONSTANTS.PI_DIV_4);
				this.pwm_8.y = this.p2.y + cache_5 * global.utils.sine(this.theta_m90 - global.CONSTANTS.PI_DIV_4);
			} else if (this.elm.flip === global.CONSTANTS.FLIP_180) {
				this.pwm_7.x = this.p1.x + cache_4 * global.utils.cosine(this.theta_m90 - global.CONSTANTS.PI_DIV_4);
				this.pwm_7.y = this.p1.y + cache_5 * global.utils.sine(this.theta_m90 - global.CONSTANTS.PI_DIV_4);
				this.pwm_8.x = this.p2.x + cache_4 * global.utils.cosine(this.theta_m90 + global.CONSTANTS.PI_DIV_4);
