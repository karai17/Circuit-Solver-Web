...
class Divider {
	...
	private p2: PointF;
	private p3: PointF;
	private divider_0: PointF;
	private divider_1: PointF;
	private divider_2: PointF;
	private divider_3: PointF;
	private divider_4: PointF;
	private divider_5: PointF;
	private divider_6: PointF;
	...
	constructor(type: number, id: number, n1: number, n2: number, n3: number) {
		...
		this.divider_0 = new PointF(0, 0);
		this.divider_1 = new PointF(0, 0);
		this.divider_2 = new PointF(0, 0);
		this.divider_3 = new PointF(0, 0);
		this.divider_4 = new PointF(0, 0);
		this.divider_5 = new PointF(0, 0);
		this.divider_6 = new PointF(0, 0);
	...
	update(): void {
		if (global.flags.flag_simulating && simulation_manager.solutions_ready && simulation_manager.simulation_step !== 0) {
			if (this.elm.consistent()) {
				...
				if (this.elm.properties['Input Voltage2'] !== 0) {
					this.elm.properties['Output Voltage'] = this.elm.properties['Input Voltage1'] / this.elm.properties['Input Voltage2'];
				} else {
	...
	stamp(): void {
		if (this.elm.consistent()) {
			engine_functions.stamp_voltage(this.elm.n3, -1, this.elm.properties['Output Voltage'], simulation_manager.ELEMENT_DIV_OFFSET + this.simulation_id);
		}
	}
	...
	build_element(): void {
		if (
			...
			let cache_6: number = 1.5 * this.x_space;
			let cache_7: number = 1.414 * this.x_space;
			let cache_8: number = 1.5 * this.y_space;
			let cache_9: number = 1.414 * this.y_space;
			this.plus_point.x = this.c_x - cache_6 * global.utils.cosine(this.theta) - cache_7 * global.utils.cosine(this.theta_m90);
			this.plus_point.y = this.c_y - cache_8 * global.utils.sine(this.theta) - cache_9 * global.utils.sine(this.theta_m90);
			this.divider_0.x = this.p1.x + cache_0 * global.utils.cosine(this.theta_m90);
			this.divider_0.y = this.p1.y + cache_1 * global.utils.sine(this.theta_m90);
			this.divider_1.x = this.divider_0.x + cache_4 * global.utils.cosine(this.theta);
			this.divider_1.y = this.divider_0.y + cache_5 * global.utils.sine(this.theta);
			this.divider_2.x = this.divider_1.x + cache_2 * global.utils.cosine(this.theta - Math.PI);
			this.divider_2.y = this.divider_1.y + cache_3 * global.utils.sine(this.theta - Math.PI);
			this.divider_3.x = this.p2.x + cache_0 * global.utils.cosine(this.theta_m90);
			this.divider_3.y = this.p2.y + cache_1 * global.utils.sine(this.theta_m90);
			this.divider_4.x = this.divider_3.x - cache_4 * global.utils.cosine(this.theta);
			this.divider_4.y = this.divider_3.y - cache_5 * global.utils.sine(this.theta);
			this.divider_5.x = this.divider_4.x + cache_2 * global.utils.cosine(this.theta);
			this.divider_5.y = this.divider_4.y + cache_3 * global.utils.sine(this.theta);
			this.divider_6.x = this.p3.x - cache_4 * global.utils.cosine(this.theta_m90);
			this.divider_6.y = this.p3.y - cache_5 * global.utils.sine(this.theta_m90);
	...
	draw_component(canvas: GraphicsEngine): void {
		...
		if (
			...
			this.circle_buffer = [];
			this.line_buffer = [];
			this.line_buffer[this.indexer++] = Array(this.p1.x, this.p1.y, this.divider_0.x, this.divider_0.y);
			this.line_buffer[this.indexer++] = Array(this.divider_0.x, this.divider_0.y, this.divider_1.x, this.divider_1.y);
			this.line_buffer[this.indexer++] = Array(this.p2.x, this.p2.y, this.divider_3.x, this.divider_3.y);
			this.line_buffer[this.indexer++] = Array(this.divider_3.x, this.divider_3.y, this.divider_4.x, this.divider_4.y);
			this.line_buffer[this.indexer++] = Array(this.c_x - this.bounds.get_width() * 0.1, this.c_y, this.c_x + this.bounds.get_width() * 0.1, this.c_y);
			this.line_buffer[this.indexer++] = Array(this.divider_6.x, this.divider_6.y, this.p3.x, this.p3.y);
			...
			this.circle_buffer[this.indexer++] = Array(this.c_x, this.c_y + this.bounds.get_width() * 0.1, 0.75 * global.variables.canvas_stroke_width_2_zoom);
			this.circle_buffer[this.indexer++] = Array(this.c_x, this.c_y - this.bounds.get_width() * 0.1, 0.75 * global.variables.canvas_stroke_width_2_zoom);
