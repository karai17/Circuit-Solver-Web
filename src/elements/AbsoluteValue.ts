...
class AbsoluteValue {
	...
	private gain_0: PointF;
	private gain_2: PointF;
	private gain_1: PointF;
	private gain_3: PointF;
	private gain_4: PointF;
	private c_x: number;
	private c_y: number;
	...
	constructor(type: number, id: number, n1: number, n2: number) {
		...
		this.elm = new Element2(id, type, global.utils.copy(global.PROPERTY.PROPERTY_ABS));
	...
	update(): void {
		if (global.flags.flag_simulating && simulation_manager.solutions_ready && simulation_manager.simulation_step !== 0) {
			if (this.elm.consistent()) {
				...
				this.elm.properties['Output Voltage'] = Math.abs(this.elm.properties['Input Voltage']);
			}
		}
	}
	stamp(): void {
	...
	build_element(): void {
		if (
			...
			this.gain_0.x = this.c_x - cache_2 * global.utils.cosine(this.theta) + cache_2 * global.utils.cosine(this.theta_m90);
			this.gain_0.y = this.c_y - cache_2 * global.utils.sine(this.theta) + cache_2 * global.utils.sine(this.theta_m90);
			this.gain_1.x = this.c_x - cache_2 * global.utils.cosine(this.theta) - cache_2 * global.utils.cosine(this.theta_m90);
			this.gain_1.y = this.c_y - cache_2 * global.utils.sine(this.theta) - cache_2 * global.utils.sine(this.theta_m90);
			this.gain_2.x = this.c_x + cache_2 * global.utils.cosine(this.theta);
			this.gain_2.y = this.c_y + cache_2 * global.utils.sine(this.theta);
			this.gain_3.x = this.c_x + cache_0 * global.utils.cosine(this.theta);
			this.gain_3.y = this.c_y + cache_0 * global.utils.sine(this.theta);
			this.gain_4.x = this.c_x - cache_1 * global.utils.cosine(this.theta);
			this.gain_4.y = this.c_y - cache_1 * global.utils.sine(this.theta);
	...
	resize(): void {
		if (this.build_element_flag || global.flags.flag_build_element) {
			...
			let temp1: number = global.variables.canvas_stroke_width_1_zoom;
			let temp2: number = global.variables.canvas_text_size_3_zoom;
			this.line_paint.set_stroke_width(global.variables.canvas_stroke_width_1_zoom);
			this.line_paint.set_text_size(global.variables.canvas_text_size_3_zoom);
			this.point_paint.set_stroke_width(global.variables.canvas_stroke_width_1_zoom);
			this.point_paint.set_text_size(global.variables.canvas_text_size_3_zoom);
	...
	draw_component(canvas: GraphicsEngine): void {
		...
		if (
			...
			this.circle_buffer = [];
			this.line_buffer = [];
			this.line_buffer[this.indexer++] = Array(this.gain_0.x, this.gain_0.y, this.gain_1.x, this.gain_1.y);
			this.line_buffer[this.indexer++] = Array(this.gain_0.x, this.gain_0.y, this.gain_2.x, this.gain_2.y);
			this.line_buffer[this.indexer++] = Array(this.gain_1.x, this.gain_1.y, this.gain_2.x, this.gain_2.y);
