...
class DifferentiatorModule {
	...
	private diff_0: PointF;
	private diff_1: PointF;
	private diff_2: PointF;
	private diff_3: PointF;
	private diff_4: PointF;
	private diff_5: PointF;
	private c_x: number;
	private c_y: number;
	...
	constructor(type: number, id: number, n1: number, n2: number) {
		...
		this.elm = new Element2(id, type, global.utils.copy(global.PROPERTY.PROPERTY_DIFFERENTIATOR));
		...
		this.diff_0 = new PointF(0, 0);
		this.diff_1 = new PointF(0, 0);
		this.diff_2 = new PointF(0, 0);
		this.diff_3 = new PointF(0, 0);
		this.diff_4 = new PointF(0, 0);
		this.diff_5 = new PointF(0, 0);
		...
		if (global.flags.flag_simulating && simulation_manager.solutions_ready && simulation_manager.simulation_step !== 0) {
			if (this.elm.consistent()) {
				if (simulation_manager.simulation_time < simulation_manager.time_step) {
					...
					this.elm.properties['Output Voltage'] = global.utils.copy(this.elm.properties['Initial Value']);
				} else {
					this.elm.properties['Last Value'] = global.utils.copy(this.elm.properties['Input Voltage']);
					this.elm.properties['Input Voltage'] = engine_functions.get_voltage(this.elm.n1, -1);
					this.elm.properties['Output Voltage'] = (this.elm.properties['Input Voltage'] - this.elm.properties['Last Value']) / simulation_manager.time_step;
				}
	...
	stamp(): void {
		if (this.elm.consistent()) {
			engine_functions.stamp_voltage(this.elm.n2, -1, this.elm.properties['Output Voltage'], simulation_manager.ELEMENT_DIFFERENTIATOR_OFFSET + this.simulation_id);
		}
	}
	...
	build_element(): void {
		if (
			...
			this.diff_0.x = this.c_x - cache_4 * global.utils.cosine(this.theta) + cache_4 * global.utils.cosine(this.theta_m90);
			this.diff_0.y = this.c_y - cache_5 * global.utils.sine(this.theta) + cache_5 * global.utils.sine(this.theta_m90);
			this.diff_2.x = this.c_x - cache_4 * global.utils.cosine(this.theta) - cache_4 * global.utils.cosine(this.theta_m90);
			this.diff_2.y = this.c_y - cache_5 * global.utils.sine(this.theta) - cache_5 * global.utils.sine(this.theta_m90);
			this.diff_3.x = this.c_x + cache_4 * global.utils.cosine(this.theta) - cache_4 * global.utils.cosine(this.theta_m90);
			this.diff_3.y = this.c_y + cache_5 * global.utils.sine(this.theta) - cache_5 * global.utils.sine(this.theta_m90);
			this.diff_4.x = this.c_x + cache_4 * global.utils.cosine(this.theta) + cache_4 * global.utils.cosine(this.theta_m90);
			this.diff_4.y = this.c_y + cache_5 * global.utils.sine(this.theta) + cache_5 * global.utils.sine(this.theta_m90);
			this.diff_5.x = this.c_x + cache_4 * global.utils.cosine(this.theta);
			this.diff_5.y = this.c_y + cache_5 * global.utils.sine(this.theta);
	...
	draw_component(canvas: GraphicsEngine): void {
		...
		if (
			...
			this.circle_buffer = [];
			this.line_buffer = [];
			this.line_buffer[this.indexer++] = Array(this.diff_0.x, this.diff_0.y, this.diff_2.x, this.diff_2.y);
			this.line_buffer[this.indexer++] = Array(this.diff_2.x, this.diff_2.y, this.diff_3.x, this.diff_3.y);
			this.line_buffer[this.indexer++] = Array(this.diff_3.x, this.diff_3.y, this.diff_4.x, this.diff_4.y);
			this.line_buffer[this.indexer++] = Array(this.diff_4.x, this.diff_4.y, this.diff_0.x, this.diff_0.y);
			...
			this.indexer = 0;
			canvas.draw_text('S', this.c_x, this.c_y, this.text_paint);
