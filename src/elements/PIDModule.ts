...
class PIDModule {
	...
	private p2: PointF;
	private p3: PointF;
	private pid_0: PointF;
	private pid_1: PointF;
	private pid_2: PointF;
	private pid_3: PointF;
	private pid_4: PointF;
	private pid_5: PointF;
	private pid_6: PointF;
	private pid_7: PointF;
	private pid_8: PointF;
	...
	constructor(type: number, id: number, n1: number, n2: number, n3: number) {
		...
		this.elm = new Element3(id, type, global.utils.copy(global.PROPERTY.PROPERTY_PID));
		...
		this.pid_7 = new PointF(0, 0);
		this.pid_8 = new PointF(0, 0);
		...
		this.m_x = 0;
		this.m_y = 0;
		this.pid_controller = new PIDController(this.elm.properties['Setpoint'], this.elm.properties['Kp'], this.elm.properties['Ki'], this.elm.properties['Kd']);
		this.pid_controller.set_output_limits(this.elm.properties['Min Output'], this.elm.properties['Max Output']);
	...
	update(): void {
		if (global.flags.flag_simulating && simulation_manager.solutions_ready && simulation_manager.simulation_step !== 0) {
			if (this.elm.consistent()) {
				...
				this.pid_controller.set_ki(this.elm.properties['Ki']);
				this.pid_controller.set_kd(this.elm.properties['Kd']);
				this.pid_controller.set_output_limits(this.elm.properties['Min Output'], this.elm.properties['Max Output']);
				...
				if (this.elm.properties['Input Voltage2'] > 0.5 * this.elm.properties['High Voltage']) {
					this.pid_controller.reset();
				}
				this.elm.properties['Output Voltage'] = this.pid_controller.get_output(simulation_manager.simulation_time, this.elm.properties['Input Voltage1']);
			}
		} else {
			this.pid_controller.reset();
		}
	...
	stamp(): void {
		if (this.elm.consistent()) {
			engine_functions.stamp_voltage(this.elm.n3, -1, this.elm.properties['Output Voltage'], simulation_manager.ELEMENT_PID_OFFSET + this.simulation_id);
		}
	}
	...
	build_element(): void {
		if (
			...
			this.pid_0.x = this.p1.x + cache_0 * global.utils.cosine(this.theta_m90);
			this.pid_0.y = this.p1.y + cache_1 * global.utils.sine(this.theta_m90);
			this.pid_1.x = this.pid_0.x + cache_6 * global.utils.cosine(this.theta);
			this.pid_1.y = this.pid_0.y + cache_7 * global.utils.sine(this.theta);
			this.pid_2.x = this.pid_1.x + cache_2 * global.utils.cosine(this.theta - Math.PI);
			this.pid_2.y = this.pid_1.y + cache_3 * global.utils.sine(this.theta - Math.PI);
			this.pid_3.x = this.p2.x + cache_0 * global.utils.cosine(this.theta_m90);
			this.pid_3.y = this.p2.y + cache_1 * global.utils.sine(this.theta_m90);
			this.pid_4.x = this.pid_3.x - cache_6 * global.utils.cosine(this.theta);
			this.pid_4.y = this.pid_3.y - cache_7 * global.utils.sine(this.theta);
			this.pid_5.x = this.pid_4.x + cache_2 * global.utils.cosine(this.theta);
			this.pid_5.y = this.pid_4.y + cache_3 * global.utils.sine(this.theta);
			this.pid_6.x = this.p3.x - cache_6 * global.utils.cosine(this.theta_m90);
			this.pid_6.y = this.p3.y - cache_7 * global.utils.sine(this.theta_m90);
			if (this.elm.flip === global.CONSTANTS.FLIP_0) {
				this.pid_7.x = this.p1.x + cache_4 * global.utils.cosine(this.theta_m90 + global.CONSTANTS.PI_DIV_4);
				this.pid_7.y = this.p1.y + cache_5 * global.utils.sine(this.theta_m90 + global.CONSTANTS.PI_DIV_4);
				this.pid_8.x = this.p2.x + cache_4 * global.utils.cosine(this.theta_m90 - global.CONSTANTS.PI_DIV_4);
				this.pid_8.y = this.p2.y + cache_5 * global.utils.sine(this.theta_m90 - global.CONSTANTS.PI_DIV_4);
			} else if (this.elm.flip === global.CONSTANTS.FLIP_180) {
				this.pid_7.x = this.p1.x + cache_4 * global.utils.cosine(this.theta_m90 - global.CONSTANTS.PI_DIV_4);
				this.pid_7.y = this.p1.y + cache_5 * global.utils.sine(this.theta_m90 - global.CONSTANTS.PI_DIV_4);
				this.pid_8.x = this.p2.x + cache_4 * global.utils.cosine(this.theta_m90 + global.CONSTANTS.PI_DIV_4);
	...
	draw_component(canvas: GraphicsEngine): void {
		...
		if (
			...
			this.circle_buffer = [];
			this.line_buffer = [];
			this.line_buffer[this.indexer++] = Array(this.p1.x, this.p1.y, this.pid_0.x, this.pid_0.y);
			this.line_buffer[this.indexer++] = Array(this.pid_0.x, this.pid_0.y, this.pid_1.x, this.pid_1.y);
			this.line_buffer[this.indexer++] = Array(this.p2.x, this.p2.y, this.pid_3.x, this.pid_3.y);
			this.line_buffer[this.indexer++] = Array(this.pid_3.x, this.pid_3.y, this.pid_4.x, this.pid_4.y);
			this.line_buffer[this.indexer++] = Array(this.pid_6.x, this.pid_6.y, this.p3.x, this.p3.y);
			...
			if (global.variables.workspace_zoom_scale > 1.085 || (!MOBILE_MODE && global.variables.workspace_zoom_scale >= 0.99)) {
				canvas.draw_text('R', this.pid_8.x, this.pid_8.y, this.text_paint);
	...
	patch(): void {
		if (!global.utils.not_null(this.pid_0)) {
			this.pid_0 = new PointF(0, 0);
		}
		if (!global.utils.not_null(this.pid_1)) {
			this.pid_1 = new PointF(0, 0);
		}
		if (!global.utils.not_null(this.pid_2)) {
			this.pid_2 = new PointF(0, 0);
		}
		if (!global.utils.not_null(this.pid_3)) {
			this.pid_3 = new PointF(0, 0);
		}
		if (!global.utils.not_null(this.pid_4)) {
			this.pid_4 = new PointF(0, 0);
		}
		if (!global.utils.not_null(this.pid_5)) {
			this.pid_5 = new PointF(0, 0);
		}
		if (!global.utils.not_null(this.pid_6)) {
			this.pid_6 = new PointF(0, 0);
		}
		if (!global.utils.not_null(this.pid_7)) {
			this.pid_7 = new PointF(0, 0);
		}
		if (!global.utils.not_null(this.pid_8)) {
			this.pid_8 = new PointF(0, 0);
		}
