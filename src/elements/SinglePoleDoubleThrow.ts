...
class SinglePoleDoubleThrow {
	...
	private p2: PointF;
	private p3: PointF;
	private spdt_0: PointF;
	private spdt_1: PointF;
	private spdt_2: PointF;
	...
	stamp(): void {
		if (this.elm.consistent()) {
			if (this.elm.properties['Switch State'] === global.CONSTANTS.ON) {
				engine_functions.stamp_resistor(this.elm.n2, this.elm.n3, this.elm.properties['Closed Resistance']);
	...
	build_element(): void {
		if (
			...
			this.spdt_1.y = this.p2.y + this.y_space * global.utils.sine(this.theta_m90);
			this.spdt_2.x = this.p3.x + this.x_space * global.utils.cosine(this.theta_m90 - Math.PI);
			this.spdt_2.y = this.p3.y + this.y_space * global.utils.sine(this.theta_m90 - Math.PI);
	...
	draw_component(canvas: GraphicsEngine): void {
		...
		if (
			...
			this.circle_buffer = [];
			this.line_buffer = [];
			this.line_buffer[this.indexer++] = Array(this.p1.x, this.p1.y, this.spdt_0.x, this.spdt_0.y);
			this.line_buffer[this.indexer++] = Array(this.p2.x, this.p2.y, this.spdt_1.x, this.spdt_1.y);
			this.line_buffer[this.indexer++] = Array(this.p3.x, this.p3.y, this.spdt_2.x, this.spdt_2.y);
			if (this.elm.properties['Switch State'] === global.CONSTANTS.OFF) {
				this.line_buffer[this.indexer++] = Array(this.spdt_0.x, this.spdt_0.y, this.spdt_2.x, this.spdt_2.y);
			} else if (this.elm.properties['Switch State'] === global.CONSTANTS.ON) {
				this.line_buffer[this.indexer++] = Array(this.spdt_1.x, this.spdt_1.y, this.spdt_2.x, this.spdt_2.y);
			}
