...
class Potentiometer {
	...
	private p2: PointF;
	private p3: PointF;
	private pot_0: PointF;
	private pot_1: PointF;
	private pot_2: PointF;
	private pot_3: PointF;
	private pot_4: PointF;
	private pot_5: PointF;
	private pot_6: PointF;
	private pot_7: PointF;
	private pot_8: PointF;
	private pot_9: PointF;
	private pot_10: PointF;
	private pot_11: PointF;
	private pot_12: PointF;
	private pot_13: PointF;
	private c_x: number;
	private c_y: number;
	...
	constructor(type: number, id: number, n1: number, n2: number, n3: number) {
		...
		this.elm = new Element3(id, type, global.utils.copy(global.PROPERTY.PROPERTY_POTENTIOMETER));
	...
	stamp(): void {
		if (this.elm.consistent()) {
			engine_functions.stamp_resistor(this.elm.n1, this.elm.n2, global.utils.limit(this.elm.properties['Wiper Percentage'], 0.01, 99.99) * 0.01 * this.elm.properties['Resistance']);
			engine_functions.stamp_resistor(this.elm.n2, this.elm.n3, (1 - global.utils.limit(this.elm.properties['Wiper Percentage'], 0.01, 99.99) * 0.01) * this.elm.properties['Resistance']);
		}
	}
	...
	build_element(): void {
		if (
			...
			let cache_6: number = 1.682 * this.x_space;
			let cache_7: number = 1.682 * this.y_space;
			let cache_8: number = 0.4 * this.x_space;
			let cache_9: number = 0.4 * this.y_space;
			let cache_10: number = this.x_space;
			let cache_11: number = this.y_space;
			this.pot_0.x = this.c_x - cache_0 * global.utils.cosine(this.theta) + (cache_10 >> 1) * global.utils.cosine(this.theta_m90);
			this.pot_0.y = this.c_y - cache_1 * global.utils.sine(this.theta) + (cache_11 >> 1) * global.utils.sine(this.theta_m90);
			this.pot_4.x = this.c_x + (cache_10 >> 1) * global.utils.cosine(this.theta_m90);
			this.pot_4.y = this.c_y + (cache_11 >> 1) * global.utils.sine(this.theta_m90);
			this.pot_2.x = this.c_x - cache_2 * global.utils.cosine(this.theta) + (cache_10 >> 1) * global.utils.cosine(Math.PI + this.theta_m90);
			this.pot_2.y = this.c_y - cache_3 * global.utils.sine(this.theta) + (cache_11 >> 1) * global.utils.sine(Math.PI + this.theta_m90);
			this.pot_6.x = this.c_x + cache_2 * global.utils.cosine(this.theta) + (cache_10 >> 1) * global.utils.cosine(Math.PI + this.theta_m90);
			this.pot_6.y = this.c_y + cache_3 * global.utils.sine(this.theta) + (cache_11 >> 1) * global.utils.sine(Math.PI + this.theta_m90);
			this.pot_8.x = this.c_x + cache_0 * global.utils.cosine(this.theta) + (cache_10 >> 1) * global.utils.cosine(this.theta_m90);
			this.pot_8.y = this.c_y + cache_1 * global.utils.sine(this.theta) + (cache_11 >> 1) * global.utils.sine(this.theta_m90);
			this.pot_7.x = this.c_x + cache_10 * global.utils.cosine(this.theta) + cache_4 * global.utils.cosine(this.theta_m90);
			this.pot_7.y = this.c_y + cache_11 * global.utils.sine(this.theta) + cache_5 * global.utils.sine(this.theta_m90);
			this.pot_9.x = this.c_x - cache_10 * global.utils.cosine(this.theta) + cache_4 * global.utils.cosine(this.theta_m90);
			this.pot_9.y = this.c_y - cache_11 * global.utils.sine(this.theta) + cache_5 * global.utils.sine(this.theta_m90);
			this.pot_10.x = this.c_x + cache_10 * global.utils.cosine(this.theta) + cache_4 * global.utils.cosine(Math.PI + this.theta_m90);
			this.pot_10.y = this.c_y + cache_11 * global.utils.sine(this.theta) + cache_5 * global.utils.sine(Math.PI + this.theta_m90);
			this.pot_11.x = this.c_x - cache_10 * global.utils.cosine(this.theta) + cache_4 * global.utils.cosine(Math.PI + this.theta_m90);
			this.pot_11.y = this.c_y - cache_11 * global.utils.sine(this.theta) + cache_5 * global.utils.sine(Math.PI + this.theta_m90);
			this.pot_12.x = this.p1.x + 4 * cache_10 * (this.elm.properties['Wiper Percentage'] * 0.01) * global.utils.cosine(this.theta) + cache_10 * global.utils.cosine(this.theta_m90);
			this.pot_12.y = this.p1.y + 4 * cache_11 * (this.elm.properties['Wiper Percentage'] * 0.01) * global.utils.sine(this.theta) + cache_11 * global.utils.sine(this.theta_m90);
			this.pot_13.x = this.p1.x + 4 * cache_10 * (this.elm.properties['Wiper Percentage'] * 0.01) * global.utils.cosine(this.theta) + cache_6 * global.utils.cosine(this.theta_m90);
			this.pot_13.y = this.p1.y + 4 * cache_11 * (this.elm.properties['Wiper Percentage'] * 0.01) * global.utils.sine(this.theta) + cache_7 * global.utils.sine(this.theta_m90);
			this.connect1_x = this.c_x - cache_10 * global.utils.cosine(this.theta);
			this.connect1_y = this.c_y - cache_11 * global.utils.sine(this.theta);
			this.connect2_x = this.c_x + cache_10 * global.utils.cosine(this.theta);
			this.connect2_y = this.c_y + cache_11 * global.utils.sine(this.theta);
			this.theta = global.utils.retrieve_angle_radian(-(this.c_x - this.p2.x), -(this.c_y - this.p2.y));
			this.pot_1.x = this.p2.x + cache_8 * global.utils.cosine(this.phi);
			this.pot_1.y = this.p2.y + cache_9 * global.utils.sine(this.phi);
			this.pot_3.x = this.pot_12.x + cache_8 * global.utils.cosine(this.theta - global.CONSTANTS.PI_DIV_6);
			this.pot_3.y = this.pot_12.y + cache_9 * global.utils.sine(this.theta - global.CONSTANTS.PI_DIV_6);
			this.pot_5.x = this.pot_12.x + cache_8 * global.utils.cosine(this.theta + global.CONSTANTS.PI_DIV_6);
			this.pot_5.y = this.pot_12.y + cache_9 * global.utils.sine(this.theta + global.CONSTANTS.PI_DIV_6);
			this.theta = global.utils.retrieve_angle_radian(this.p3.x - this.p1.x, this.p3.y - this.p1.y);
		...
		if (
			...
			if (global.variables.workspace_zoom_scale > 1.085 || (!MOBILE_MODE && global.variables.workspace_zoom_scale >= 0.99)) {
				...
				if (this.angle > 170 && this.angle < 190) {
					canvas.draw_text(
						global.TEMPLATES.ELEMENT_VAL_TEMPLATE.replace('{VAL}', global.utils.exponentiate_quickly(this.elm.properties['Resistance'])).replace('{UNIT}', this.elm.properties['units']),
						this.c_x,
