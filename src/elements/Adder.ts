...
class Adder {
	...
	private p2: PointF;
	private p3: PointF;
	private adder_0: PointF;
	private adder_1: PointF;
	private adder_2: PointF;
	private adder_3: PointF;
	private adder_4: PointF;
	private adder_5: PointF;
	private adder_6: PointF;
	...
	constructor(type: number, id: number, n1: number, n2: number, n3: number) {
		...
		this.elm = new Element3(id, type, global.utils.copy(global.PROPERTY.PROPERTY_ADD));
		...
		this.adder_0 = new PointF(0, 0);
		this.adder_1 = new PointF(0, 0);
		this.adder_2 = new PointF(0, 0);
		this.adder_3 = new PointF(0, 0);
		this.adder_4 = new PointF(0, 0);
		this.adder_5 = new PointF(0, 0);
		this.adder_6 = new PointF(0, 0);
	...
	stamp(): void {
		if (this.elm.consistent()) {
			engine_functions.stamp_voltage(this.elm.n3, -1, this.elm.properties['Output Voltage'], simulation_manager.ELEMENT_ADD_OFFSET + this.simulation_id);
		}
	}
	...
	build_element(): void {
		if (
			...
			this.adder_0.x = this.p1.x + cache_0 * global.utils.cosine(this.theta_m90);
			this.adder_0.y = this.p1.y + cache_1 * global.utils.sine(this.theta_m90);
			this.adder_1.x = this.adder_0.x + cache_4 * global.utils.cosine(this.theta);
			this.adder_1.y = this.adder_0.y + cache_5 * global.utils.sine(this.theta);
			this.adder_2.x = this.adder_1.x + cache_2 * global.utils.cosine(this.theta - Math.PI);
			this.adder_2.y = this.adder_1.y + cache_3 * global.utils.sine(this.theta - Math.PI);
			this.adder_3.x = this.p2.x + cache_0 * global.utils.cosine(this.theta_m90);
			this.adder_3.y = this.p2.y + cache_1 * global.utils.sine(this.theta_m90);
			this.adder_4.x = this.adder_3.x - cache_4 * global.utils.cosine(this.theta);
			this.adder_4.y = this.adder_3.y - cache_5 * global.utils.sine(this.theta);
			this.adder_5.x = this.adder_4.x + cache_2 * global.utils.cosine(this.theta);
			this.adder_5.y = this.adder_4.y + cache_3 * global.utils.sine(this.theta);
			this.adder_6.x = this.p3.x - cache_4 * global.utils.cosine(this.theta_m90);
			this.adder_6.y = this.p3.y - cache_5 * global.utils.sine(this.theta_m90);
	...
	draw_component(canvas: GraphicsEngine): void {
		...
		if (
			...
			this.circle_buffer = [];
			this.line_buffer = [];
			this.line_buffer[this.indexer++] = Array(this.p1.x, this.p1.y, this.adder_0.x, this.adder_0.y);
			this.line_buffer[this.indexer++] = Array(this.adder_0.x, this.adder_0.y, this.adder_1.x, this.adder_1.y);
			this.line_buffer[this.indexer++] = Array(this.p2.x, this.p2.y, this.adder_3.x, this.adder_3.y);
			this.line_buffer[this.indexer++] = Array(this.adder_3.x, this.adder_3.y, this.adder_4.x, this.adder_4.y);
			...
			this.line_buffer[this.indexer++] = Array(this.c_x, this.c_y + this.bounds.get_width() * 0.1, this.c_x, this.c_y - this.bounds.get_width() * 0.1);
			this.line_buffer[this.indexer++] = Array(this.adder_6.x, this.adder_6.y, this.p3.x, this.p3.y);
