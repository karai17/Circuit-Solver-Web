...
class VoltageControlledSwitch {
	...
	private p2: PointF;
	private p3: PointF;
	private vcsw_0: PointF;
	private vcsw_1: PointF;
	private vcsw_2: PointF;
	private vcsw_4: PointF;
	private vcsw_3: PointF;
	private c_x: number;
	private c_y: number;
	...
	constructor(type: number, id: number, n1: number, n2: number, n3: number) {
		...
		this.elm = new Element3(id, type, global.utils.copy(global.PROPERTY.PROPERTY_VCSW));
	...
	stamp(): void {
		if (this.elm.consistent()) {
			if (this.elm.properties['Output Voltage'] < this.elm.properties['High Voltage'] * 0.5) {
				engine_functions.stamp_resistor(this.elm.n1, this.elm.n3, this.elm.properties['Closed Resistance']);
			}
		}
	}
	...
	build_element(): void {
		if (
			...
			let cache_2: number = 0.8 * this.x_space;
			let cache_3: number = 0.8 * this.y_space;
			let cache_4: number = 0.4 * this.x_space;
			let cache_5: number = 0.4 * this.y_space;
			...
			this.vcsw_0.x = this.connect1_x + cache_6 * global.utils.cosine(this.theta) + cache_0 * global.utils.cosine(this.theta_m90);
			this.vcsw_0.y = this.connect1_y + cache_7 * global.utils.sine(this.theta) + cache_1 * global.utils.sine(this.theta_m90);
			this.theta = global.utils.retrieve_angle_radian(-(this.c_x - this.p2.x), -(this.c_y - this.p2.y));
			this.vcsw_1.x = this.p2.x + cache_2 * global.utils.cosine(this.phi);
			this.vcsw_1.y = this.p2.y + cache_3 * global.utils.sine(this.phi);
			this.vcsw_2.x = this.vcsw_1.x + cache_4 * global.utils.cosine(this.theta - global.CONSTANTS.PI_DIV_6);
			this.vcsw_2.y = this.vcsw_1.y + cache_5 * global.utils.sine(this.theta - global.CONSTANTS.PI_DIV_6);
			this.vcsw_3.x = this.vcsw_1.x + cache_4 * global.utils.cosine(this.theta + global.CONSTANTS.PI_DIV_6);
			this.vcsw_3.y = this.vcsw_1.y + cache_5 * global.utils.sine(this.theta + global.CONSTANTS.PI_DIV_6);
			this.theta = global.utils.retrieve_angle_radian(this.p3.x - this.p1.x, this.p3.y - this.p1.y);
	...
	draw_component(canvas: GraphicsEngine): void {
		...
		if (
			...
			this.line_buffer[this.indexer++] = Array(this.p2.x, this.p2.y, this.vcsw_1.x, this.vcsw_1.y);
			this.line_buffer[this.indexer++] = Array(this.vcsw_2.x, this.vcsw_2.y, this.vcsw_1.x, this.vcsw_1.y);
			this.line_buffer[this.indexer++] = Array(this.vcsw_3.x, this.vcsw_3.y, this.vcsw_1.x, this.vcsw_1.y);
