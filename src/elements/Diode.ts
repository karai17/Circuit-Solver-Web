...
class Diode {
	...
	private dio_0: PointF;
	private dio_1: PointF;
	private dio_2: PointF;
	private dio_3: PointF;
	private c_x: number;
	private c_y: number;
	...
	constructor(type: number, id: number, n1: number, n2: number) {
		...
		this.elm = new Element2(id, type, global.utils.copy(global.PROPERTY.PROPERTY_DIODE));
	...
	update(): void {
		if (global.flags.flag_simulating && simulation_manager.solutions_ready) {
			if (this.elm.consistent()) {
				...
				} else if (next_voltage < -this.damping_safety_factor * vcrit) {
					diode_voltage = global.utils.log_damping(next_voltage, this.elm.properties['Voltage'], this.gamma, this.kappa);
				} else {
					diode_voltage = next_voltage;
				}
				diode_voltage = global.utils.limit(diode_voltage, -vcrit, vcrit);
				this.gmin_step(this.gmin_start, this.get_diode_error());
				...
				this.elm.properties['Equivalent Current'] = -(
					this.elm.properties['Saturation Current'] * (Math.exp(this.elm.properties['Voltage'] / (this.elm.properties['Emission Coefficient'] * global.settings.THERMAL_VOLTAGE)) - 1) -
	...
	build_element(): void {
		if (
			...
			this.dio_0.y = this.c_y + cache_1 * global.utils.sine(this.theta) + cache_1 * global.utils.sine(this.theta_m90);
			this.dio_1.x = this.c_x - cache_0 * global.utils.cosine(this.theta) + cache_0 * global.utils.cosine(this.theta_m90);
			this.dio_1.y = this.c_y - cache_1 * global.utils.sine(this.theta) + cache_1 * global.utils.sine(this.theta_m90);
			this.dio_3.x = this.c_x + cache_0 * global.utils.cosine(this.theta) + cache_0 * global.utils.cosine(Math.PI + this.theta_m90);
			this.dio_3.y = this.c_y + cache_1 * global.utils.sine(this.theta) + cache_1 * global.utils.sine(Math.PI + this.theta_m90);
			this.dio_2.x = this.c_x - cache_0 * global.utils.cosine(this.theta) + cache_0 * global.utils.cosine(Math.PI + this.theta_m90);
			this.dio_2.y = this.c_y - cache_1 * global.utils.sine(this.theta) + cache_1 * global.utils.sine(Math.PI + this.theta_m90);
			this.connect1_x = this.c_x - cache_0 * global.utils.cosine(this.theta);
			this.connect1_y = this.c_y - cache_1 * global.utils.sine(this.theta);
			this.connect2_x = this.c_x + cache_0 * global.utils.cosine(this.theta);
			this.connect2_y = this.c_y + cache_1 * global.utils.sine(this.theta);
	...
	draw_component(canvas: GraphicsEngine): void {
		...
		if (
			...
			this.circle_buffer = [];
			this.line_buffer = [];
			this.line_buffer[this.indexer++] = Array(this.dio_1.x, this.dio_1.y, this.dio_2.x, this.dio_2.y);
			this.line_buffer[this.indexer++] = Array(this.dio_3.x, this.dio_3.y, this.dio_0.x, this.dio_0.y);
			this.line_buffer[this.indexer++] = Array(this.dio_1.x, this.dio_1.y, this.connect2_x, this.connect2_y);
			this.line_buffer[this.indexer++] = Array(this.dio_2.x, this.dio_2.y, this.connect2_x, this.connect2_y);
