...
class Rail {
	...
	constructor(type: number, id: number, n1: number) {
		...
		this.elm = new Element1(id, type, global.utils.copy(global.PROPERTY.PROPERTY_RAIL));
		...
		this.elm.set_rotation(global.CONSTANTS.ROTATION_180);
		this.release_nodes();
		...
		if (
			...
			if (!global.variables.focused && !global.variables.component_touched && !global.variables.multi_selected) {
				if (this.elm.consistent() && !global.variables.component_touched) {
					...
					} else if (this.bounds.contains_xywh(global.variables.mouse_x, global.variables.mouse_y, this.bounds.get_width() * 0.86956, this.bounds.get_height() * 0.86956)) {
						this.is_translating = false;
