...
class DACModule {
	...
	private dac_0: PointF;
	private dac_1: PointF;
	private dac_2: PointF;
	private dac_3: PointF;
	private dac_4: PointF;
	private dac_5: PointF;
	private c_x: number;
	private c_y: number;
	...
	constructor(type: number, id: number, n1: number, n2: number) {
		...
		this.elm = new Element2(id, type, global.utils.copy(global.PROPERTY.PROPERTY_DAC));
		...
		this.dac_0 = new PointF(0, 0);
		this.dac_1 = new PointF(0, 0);
		this.dac_2 = new PointF(0, 0);
		this.dac_3 = new PointF(0, 0);
		this.dac_4 = new PointF(0, 0);
		this.dac_5 = new PointF(0, 0);
	...
	update(): void {
		if (global.flags.flag_simulating && simulation_manager.solutions_ready && simulation_manager.simulation_step !== 0) {
			if (this.elm.consistent()) {
				...
				this.elm.properties['Max Bits'] = Math.pow(2, this.elm.properties['Bit Resolution']) - 1;
				this.elm.properties['LSB'] = this.elm.properties['Reference Voltage'] / this.elm.properties['Max Bits'];
				this.elm.properties['Output Voltage'] = global.utils.limit(
					this.elm.properties['Input Voltage'] * this.elm.properties['LSB'],
					0,
					this.elm.properties['Reference Voltage'] - this.elm.properties['LSB']
				);
			}
		}
	}
	stamp(): void {
		if (this.elm.consistent()) {
			engine_functions.stamp_voltage(this.elm.n2, -1, this.elm.properties['Output Voltage'], simulation_manager.ELEMENT_DAC_OFFSET + this.simulation_id);
		}
	}
	...
	build_element(): void {
		if (
			...
			this.dac_0.x = this.c_x - cache_4 * global.utils.cosine(this.theta) + cache_4 * global.utils.cosine(this.theta_m90);
			this.dac_0.y = this.c_y - cache_5 * global.utils.sine(this.theta) + cache_5 * global.utils.sine(this.theta_m90);
			this.dac_2.x = this.c_x - cache_4 * global.utils.cosine(this.theta) - cache_4 * global.utils.cosine(this.theta_m90);
			this.dac_2.y = this.c_y - cache_5 * global.utils.sine(this.theta) - cache_5 * global.utils.sine(this.theta_m90);
			this.dac_3.x = this.c_x + cache_4 * global.utils.cosine(this.theta) - cache_4 * global.utils.cosine(this.theta_m90);
			this.dac_3.y = this.c_y + cache_5 * global.utils.sine(this.theta) - cache_5 * global.utils.sine(this.theta_m90);
			this.dac_4.x = this.c_x + cache_4 * global.utils.cosine(this.theta) + cache_4 * global.utils.cosine(this.theta_m90);
			this.dac_4.y = this.c_y + cache_5 * global.utils.sine(this.theta) + cache_5 * global.utils.sine(this.theta_m90);
			this.dac_5.x = this.c_x + cache_4 * global.utils.cosine(this.theta);
			this.dac_5.y = this.c_y + cache_5 * global.utils.sine(this.theta);
	...
	draw_component(canvas: GraphicsEngine): void {
		...
		if (
			...
			this.circle_buffer = [];
			this.line_buffer = [];
			this.line_buffer[this.indexer++] = Array(this.dac_0.x, this.dac_0.y, this.dac_2.x, this.dac_2.y);
			this.line_buffer[this.indexer++] = Array(this.dac_2.x, this.dac_2.y, this.dac_3.x, this.dac_3.y);
			this.line_buffer[this.indexer++] = Array(this.dac_3.x, this.dac_3.y, this.dac_4.x, this.dac_4.y);
			this.line_buffer[this.indexer++] = Array(this.dac_4.x, this.dac_4.y, this.dac_0.x, this.dac_0.y);
			...
			this.indexer = 0;
			canvas.draw_text('D/A', this.c_x, this.c_y, this.text_paint);
