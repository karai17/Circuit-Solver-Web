...
class SettingsWindow {
	...
	private readonly ATTRIBUTE_SHOW_SIZE: number;
	...
	mouse_down(): void {
		if (global.flags.flag_select_settings && !global.variables.is_right_click) {
			if (
			...
			if (!global.variables.mouse_keyboard_lock) {
				if (this.window_anchored) {
					if (
						...
						if (
							this.null_index_check(this.attributes, global.CONSTANTS.SYSTEM_OPTION_LANGUAGE) &&
							...
							this.null_index_check(this.attributes, global.CONSTANTS.SYSTEM_OPTION_STRETCH_WINDOW) &&
				...
				if (global.utils.not_null(global.variables.system_options)) {
					if (i < global.variables.system_options['options'].length && global.utils.not_null(this.attributes[i])) {
						...
						if (i === global.CONSTANTS.SYSTEM_OPTION_LANGUAGE) {
							...
							canvas.draw_text(
								language_manager.SHORTCUT_HINTS[global.CONSTANTS.LANGUAGES[global.variables.language_index]] + ':=',
								this.attributes[i].left + this.PADDING * this.bounds.get_width() + this.offset_x,
								this.attributes[i].get_center_y() + this.offset_y,
								this.text_paint
							);
							...
							canvas.draw_text(
								language_manager.STRETCH_WINDOW[global.CONSTANTS.LANGUAGES[global.variables.language_index]] + ':=',
								this.attributes[i].left + this.PADDING * this.bounds.get_width() + this.offset_x,
								this.attributes[i].get_center_y() + this.offset_y,
								this.text_paint
							);
							...
							canvas.draw_text(
								global.variables.system_options['options'][i] + ':=',
						...
						if (global.variables.system_options['values'][i] === global.CONSTANTS.ON || global.variables.system_options['values'][i] === global.CONSTANTS.OFF) {
							...
							canvas.draw_text(
								global.variables.system_options['values'][i],
